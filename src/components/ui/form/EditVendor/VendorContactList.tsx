...
const VendorContactList = ({ item, index, showButton, formik }: Props) => {
  ...
  const contacts = formik.values.vendorPhoneNumbers;

  const DeleteVendorPhone = useMutation({
    mutationFn: async (id: string | number | undefined) =>
      await vendorService.VendorPhoneDelete(pid, id, user),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['vendorPhone'] });
      setIsDisable(true);
    },
  ...
  const handleFormChange = (
    ...
    formik.setFieldValue('vendorPhoneNumbers', newList);
  };

  const CreateVendorPhone = useMutation({
    mutationFn: async (body: any) => {
      await vendorService.CreateVendorPhone(body, pid, user);
      return body;
    },
    onSuccess: data => {
  ...
  const UpdateVendorPhone = useMutation({
    mutationFn: async ({ body, id }: { body: any; id: string | number }) => {
      await vendorService.UpdateVendorPhoneAxios(body, pid, id, user);
      return body;
    },
    onSuccess: data => {
  ...
  const handleDeleteContact = (item: any, i: number) => {
    ...
    if (item.id) DeleteVendorPhone.mutate(item.id);
    setOpen(false);
  };
  ...
  const removeContact = (item: any, index: number) => {
    ...
    if (contacts.length !== 1) {
      ...
      formik.setFieldValue('vendorPhoneNumbers', filteredList);
  ...
  const addContact = () => {
    ...
    formik.setFieldValue('vendorPhoneNumbers', [...contacts, newObject]);
  };
          ...
          {showButton && (
            <Button
              ...
              onClick={() => {
                ...
                if (item.id) {
                  UpdateVendorPhone.mutate({ body: item, id: item.id });
                } else {
                  CreateVendorPhone.mutate(item);
                }
              }}
              sx={{
                mx: 1,
